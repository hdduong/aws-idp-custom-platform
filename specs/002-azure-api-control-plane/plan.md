...
The API calls unsigned STS `AssumeRoleWithWebIdentity`, caches the returned credentials behind a concurrency-safe provider, and refreshes before the five-minute safety window. In the initial compatibility seam, the retained domain's module-global DynamoDB/S3/Lambda clients are rebound to the refreshable workload session inside a per-replica `domain_lock`; the original browser bearer token is never forwarded to STS or AWS.

### Runtime composition

Refactor the existing lifecycle behavior in `services/loan_api/app.py` behind a host-callable dispatch entry point and configurable AWS session, preserving its already tested state transitions during the hosting move. The dispatch retains a normalized Lambda-compatible envelope and AWS-aware DynamoDB/S3/Lambda operations; before each serialized call, Azure acquires the workload session and rebinds those clients to its refreshable credentials. `services/azure_api/` supplies the FastAPI transport, CORS, correlation IDs, bounded bodies, health/readiness, cryptographic JWT verification, refreshable managed-identity/STS session, and a per-replica lock around the entire domain call. Long-lived boto3 clients use botocore refreshable credentials with synchronized early refresh; they are never initialized from a one-time static credential dictionary. Because the lock serializes domain calls, the Container Apps HTTP scale target is fixed at `1` to minimize head-of-line waiting while `apiMaxReplicas` retains horizontal scale. That target is an autoscaling signal rather than a hard admission cap, so the lock—not the ingress metric—is the correctness boundary. The legacy Lambda adapter may remain temporarily for rollback testing, but no AWS infrastructure deploys it.
...
CloudFormation handler stabilization must use the same resource identity modeled by bootstrap IAM. The registry table and source bucket therefore declare the deterministic physical names already authorized to the platform execution role. The active AWS Backup vault create-handler schema additionally calls the permission-only `backup-storage:Mount` and `backup-storage:MountCapsule` actions; the platform role grants exactly those dependencies on `*`, the service-defined resource scope, while existing `kms:*` access remains limited to platform-tagged data keys. Because cfn-lint 1.53 has not cataloged `Mount`, the execution-role resource suppresses only W3037; the structured validator rejects any broader linter exception while enforcing the exact provider-schema action pair. Backup selection creation may create the service-linked role only for `backup.amazonaws.com` and only at the AWS-managed `AWSServiceRoleForBackup` path. Structured repository validation parses both CloudFormation templates and rejects divergent names, global or relocated DynamoDB/S3 grants, standalone policy attachments, missing or broader handler actions, and generalized service-linked-role creation. Deterministic naming is a first-install correction: an existing stack with generated retained resources is not auto-migrated, and the mandatory stateful stack policy blocks replacement until a separate reviewed migration or import is supplied.
